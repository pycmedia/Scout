...
NICHES = [
    ...
    (re.compile(r"cafe|caf\u00e9|restaurant|food|bakery|coffee|kitchen|eatery", re.I), {
        "label": "Cafes & Food",
...
<div class="wrap">
  ...
  <div id="view-clients" style="display:none">
    ...
    <div class="panel">
      ...
      <div class="muted small" style="margin-top:10px">Clients are saved in this browser and power the Post ideas, Reel ideas &amp; Playbook tabs.</div>
    </div>
...
function show(tab){
  ...
  ["today","posts","reels","playbook","clients"].forEach(v=>{
...
function renderToday(){
  ...
  let h = '<div class="row" style="margin-top:16px"><button class="btn small" onclick="exportCSV()">⬇ Export CSV (Google Sheets)</button>'+
...
function removeClient(i){
  const list = clients(); list.splice(i,1); saveClients(list);
  renderClients(); NICHE=null; loadNiche(false);
  if(BRIEF) renderToday();
}
